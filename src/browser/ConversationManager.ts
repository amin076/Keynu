...
export class ConversationManager {
  ...
  async readLatestAssistantMessage(): Promise<AssistantMessageSnapshot | null> {
    ...
    try {
      const id = await latest.getAttribute("data-message-id", { timeout: 3000 });
      const text = await latest.textContent({ timeout: 3000 });
    ...
    return this.page.evaluate(
      ({ afterMessageId, stableMs }) =>
        new Promise<AssistantMessageSnapshot>((resolve) => {
          const selector = '[data-message-author-role="assistant"][data-message-id]';
          ...
          const inspect = () => {
            ...
            if (!id || id === afterMessageId || !text.trim()) return;
  ...
  async readLatestAssistantText(): Promise<string | null> {
    return (await this.readLatestAssistantMessage())?.text ?? null;
  }
  ...
  private async getMessageInput(): Promise<Locator> {
    ...
    throw new Error("ChatGPT message input was not found");
  }
